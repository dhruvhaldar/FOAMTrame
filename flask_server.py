...
def run_startup_checks():
    ...
    try:
        ...
        if not client:
            STARTUP_STATUS = {
                "status": "failed",
                "message": "Cannot connect to Docker daemon. Make sure Docker Desktop is running.",
            }
        ...
        try:
            ...
            if images:
                ...
                STARTUP_STATUS = {
                    "status": "warning",
                    "message": f"Docker image '{DOCKER_IMAGE}' not found on host.",
                }
            ...
            STARTUP_STATUS = {
                "status": "warning",
                "message": f"Docker image check failed: {e}",
            }
    except Exception as e:
...
def set_case():
    ...
    try:
        ...
        return jsonify(
            {"output": f"INFO::Case root set to: {CASE_ROOT}", "caseDir": CASE_ROOT}
        )
    except Exception as e:
